...
//Smart contract that allows owner to destroy it and transfer the funds in the contract to a custom address
contract DestroyContract{
    ...
    //function to destroy the contract and transfer the funds to another account - '_to' , can only be done by owner
    function destroyContract(address payable _to)public {

        //only destroy contract if person calling the destroy function is the owner
        require(msg.sender == owner,"Only owner can perform this function. You are not the owner");
        
        //use 'selfsdestruct() inbuilt function to destroy the contract and transfer the awailable funds to a specified address (_to)
        selfdestruct(_to);
    }
}
